...
class SharepointLinkItemAddV1
  def initialize(input)
    ...
    if @parameters['url'] == ''
      raise StandardError, "Invalid Entry: Missing URL"
    end
  end
  ...
  def execute()
    ...
    link = build_new_link_xml(
      'URL' => @parameters['url'],
      'Comments' => @parameters['notes']
    )

    # Send the request
    begin
      puts "Making the SOAP call to create the new link" if @enable_debug_logging
      # Define a block local variable so that it is included in the bindings for the 
      # block below
      link_list = @parameters['link_list']
      response = client.request(:update_list_items) do
        soap.body = Gyoku.xml({:"ins0:listName!" => link_list, :"ins0:updates!" => link})
      end
      ...
      if result[:error_code] != "0x00000000"
        if result[:error_code] == "0x81020014"
          raise StandardError, "Invalid List: '#{@parameters['link_list']}' is not a valid link list"
        else
  ...
  def build_new_link_xml(field_values)
    puts "Building the Batch XML that contains the information for the new link" if @enable_debug_logging
