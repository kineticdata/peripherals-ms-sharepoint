...
class SharepointListItemAddV1
  ...
  def execute()
    ...
    puts "Attempting to create the following item: #{list_item}" if @enable_debug_logging
    begin
      response = resource["/_api/web/lists/GetByTitle('#{URI.encode(@parameters['list_name'])}')/items"].post(
        list_item.to_json,
        {
    ...
    json = JSON.parse(response.body)
    item_id = json["d"]["Id"]
    puts "Item '#{item_id}' successfully created" if @enable_debug_logging

    return <<-RESULTS
    <results>
      <result name="Id">#{item_id}</result>
    </results>
    RESULTS
  end
