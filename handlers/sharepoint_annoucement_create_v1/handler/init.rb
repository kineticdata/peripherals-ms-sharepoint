...
class SharepointAnnouncementCreateV1
  ...
  def execute()
    ...
    announcement = build_new_announcement_xml(
      'Title' => @parameters['title'],
      'Body' => @parameters['description'],
      'Expires' => @parameters['expire_date']
    )

    # Send the request
    begin
      puts "Making the SOAP call to create the new announcement" if @enable_debug_logging
      # Define a block local variable so that it is included in the bindings for the 
      # block below
      announcement_list = @parameters['announcement_list']
      puts announcement_list
      response = client.request(:update_list_items) do
        soap.body = Gyoku.xml({:"ins0:listName!" => announcement_list, :"ins0:updates!" => announcement})
      end
      ...
      if result[:error_code] != "0x00000000"
        if result[:error_code] == "0x81020014"
          raise StandardError, "Invalid List: '#{@parameters['announcement_list']}' is not a valid announcement list"
        else
  ...
  def build_new_announcement_xml(field_values)
    puts "Building the Batch XML that contains the information for the new announcement" if @enable_debug_logging
