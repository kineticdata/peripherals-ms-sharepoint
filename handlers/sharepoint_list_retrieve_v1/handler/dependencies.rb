...
if not defined?(JSON)
  ...
  handler_path = File.expand_path(File.dirname(__FILE__))
  # Calculate the location of our library and add it to the Ruby load path
  library_path = File.join(handler_path, 'vendor/json-1.7.3-java/lib')
  $:.unshift library_path
  # Require the library
  require 'json'
end
...
elsif JSON::VERSION != '1.7.3'
  raise "Incompatible library version #{JSON::VERSION} for JSON.  Expecting version 1.7.3."
end
