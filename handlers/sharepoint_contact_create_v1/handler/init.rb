...
class SharepointContactCreateV1
  def initialize(input)
    ...
    if @parameters['last_name'] == ''
      raise StandardError, "Invalid Entry: Missing Last Name"
    end
  end
  ...
  def execute()
    ...
    contact = build_new_contact_xml(
      'Title' => @parameters['last_name'],
      'FirstName' => @parameters['first_name'],
      'Email' => @parameters['email'],
      'Company' => @parameters['company'],
      'JobTitle' => @parameters['job_title'],
      'WorkPhone' => @parameters['work_phone'],
      'HomePhone' => @parameters['home_phone'],
      'CellPhone' => @parameters['cell_phone']
    )

    # Send the request
    begin
      puts "Making the SOAP call to create the new contact" if @enable_debug_logging
      # Define a block local variable so that it is included in the bindings for the 
      # block below
      contact_list = @parameters['contact_list']
      response = client.request(:update_list_items) do
        soap.body = Gyoku.xml({:"ins0:listName!" => contact_list, :"ins0:updates!" => contact})
      end
      ...
      if result[:error_code] != "0x00000000"
        if result[:error_code] == "0x81020014"
          raise StandardError, "Invalid List: '#{@parameters['contact_list']}' is not a valid contact list"
        else
  ...
  def build_new_contact_xml(field_values)
    puts "Building the Batch XML that contains the information for the new contact" if @enable_debug_logging
